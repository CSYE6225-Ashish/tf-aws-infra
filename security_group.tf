resource "aws_security_group" "webapp_loadbalancer_sg" {
  name   = "webapp-loadbalancer-sg"
...
resource "aws_security_group" "webapp_application_sg" {
  name   = "webapp_application_sg"
  vpc_id = aws_vpc.main.id
